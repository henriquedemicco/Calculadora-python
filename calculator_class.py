...
class Calculator:
    ...
    def _config_buttons(self):
        ...
        for row_values in buttons:
            for button in row_values:
                ...
                if button_text in '+-/*^':
                    button.bind('<Button-1>', self.add_text_to_display)
                    button.config(bg='#333333', fg='#fff')
                
                if button_text == '=':
                    button.bind('<Button-1>', self.calculate)
                    button.config(bg='#5F9F9F', fg='#fff')
    ...
    def _fix_text(self, text):
        #Substitui tudo o que não for 0123456789./*-+^ para nada
    ...
    def calculate(self, event=None):
        ...
        try:
            ...
            self.label.config(text='Não consegui realizar essa conta, desculpe!')
        except Exception:
